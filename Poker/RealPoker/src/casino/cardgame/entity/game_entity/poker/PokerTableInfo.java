 ...
 */
public final class PokerTableInfo {
    //Tiền cược

    private String m_roomName;
    private String m_dealer;
    private String m_smallBlind;
    private String m_bigBlind;
...
//    private String m_bettingUser;
    ...
    private List<String> m_listUserSitOut;
...
//    private int MAX_USER = 12;
    ...
    public PokerTableInfo(String roomName, double betChip) {
        ...
        m_listUserSitOut = new ArrayList<String>();
...
////                m_mapPokerComparer.put(user.getName(), new PokerHandComparer());
//
//                return true;
...
//            Logger.error(m_PokerCardCollection.getClass(), ex);
    ...
    public Boolean StandUserUp(User user) {
        ...
        if (desk != null) {
            ...
            m_listUserSitOut.remove(user.getName());

            if (m_dealer.equals(username)) {
                m_dealer = "";
            } else if (m_smallBlind.equals(username)) {
                m_smallBlind = "";
            } else if (m_bigBlind.equals(username)) {
                m_bigBlind = "";
            }
    ...
    public void sitUserOut(String userName, boolean isSitOut) {
        if (isSitOut) {
            if (m_listUserSitOut.contains(userName) == false) {
                m_listUserSitOut.add(userName);
            }
        } else {
            m_listUserSitOut.remove(userName);
        }
    }

    public void processSittingOutUser() {
        for (int i = 0; i < m_listUserSitOut.size(); i++) {
            String userName = m_listUserSitOut.get(i);
            m_listActiveUserName.remove(userName);
        }
    }
    ...
    public void setupSmallBlindUser(){
        m_sBlindPos = getNextSmallBlindPosition(m_sBlindPos);
        if (m_sBlindPos == -1) {
            m_smallBlind = "";
        } else {
            m_smallBlind = m_listDesk.get(m_sBlindPos).getUser().getName();
        }
        m_bigBlind = "";
        m_dealer = "";
    }
...
//        m_dealer = "";
    ...
    public boolean checkDeskValid(Desk desk) {
        ...
        //if this user isnot contain in listActiveUserName && sitting on this desk -> user sitting out.
        if (m_listActiveUserName.contains(desk.getUser().getName()) == false) {
            return false;
        }
    ...
    public void processUserReady(String userName) {
        m_mapUserDesk.get(userName).setDeskState(DeskState.READY);
        if (!m_listActiveUserName.contains(userName)) {
            m_listActiveUserName.add(userName);
        }
    }
    ...
    //          case: After user check ready to start game
    public void ProcessUserPlay(String userName) {
        m_mapUserDesk.get(userName).setDeskState(DeskState.PLAYING);
    }
    //***********************************************************************************
    //      SangDN: Proces User to Play State       
    //          case: After user check ready to start game

    public void ProcessUserWaiting(String userName) {
        m_mapUserDesk.get(userName).setDeskState(DeskState.WAITING);
    }
    //***********************************************************************************
    //      khoatd: Proces User to Play State       
    //          case: After user check ready to start game
...
//            if (chip > m_bettingChip) {
    ...
    public void processSetSmallBlind(String smallBlind) {
        ...
        processUserAction(PokerGameTurn.BETTING, smallBlind, UserActionType.BETTING, getBetChipGame());

        String bigBlind = getNextPlayerName(smallBlind);
        m_bigBlind = bigBlind;
        processUserAction(PokerGameTurn.BETTING, bigBlind, UserActionType.BETTING, getBetChipGame() * 2);
        m_bettingChip = getBetChipGame() * 2;
...
//        m_numPlaying = m_listActiveUserName.size();
    ...
    public ArrayList<String> findListWinner() {
        ...
        int nComCard = m_listCommunityCard.size();
        try {
            if (nComCard < 5 || mapUserPokerHand == null) {
                return null;
            }
            ...
            if (winnerPokerHand != null) {
                // Tìm những user có poker hand == winnerPokerHand đưa vào listWinnerId
                for (int i = 0; i < m_listActiveUserName.size(); i++) {
                    String userName = m_listActiveUserName.get(i);
                    UserBetObject betObj = m_mapUserBet.get(userName).getLastAction();
                    if (betObj.getActionType().equals(UserActionType.FOLD) == false) {
    ...
    public boolean isUserFold(String userName){
        UserBetObject betObj = m_mapUserBet.get(userName).getLastAction();
        if(betObj != null && betObj.getActionType().equals(UserActionType.FOLD)){
            return true;
        }
        return false;
    }
    ...
    public int countUserNotSitOut(){
        int count = 0;
        for (int i = 0; i < m_listDesk.size(); i++) {
            User user = m_listDesk.get(i).getUser();
            if(user != null && !m_listUserSitOut.contains(user.getName())){
                count++;
            }
        }
            ...
            for (i = 0; i < m_listActiveUserName.size(); i++) {
                ...
                if (userCards != null) {
                    for (int j = 0; j < userCards.size(); j++) {
                        listUserCard += userCards.get(i).getCardName();
                        listUserCard += "; ";
                    }
    ...
    public void setSmallBlind(String m_smallBlind) {
        this.m_smallBlind = m_smallBlind;
    }
    ...
    public void setBigBlind(String m_bigBlind) {
        this.m_bigBlind = m_bigBlind;
    }

    public double getBetChipGame() {
        return m_betChipGame;
    }

    public void setBetChipGame(double m_betChip) {
        this.m_betChipGame = m_betChip;
    }
    ...
    public HashMap<String, Boolean> getMapUserBetStatus() {
        return m_mapUserBetStatus;
    }
    ...
    public List<String> getListUserSitOut() {
